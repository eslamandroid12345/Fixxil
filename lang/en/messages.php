<?php

return [
    'No data found' => 'No data found.',
    'Incorrect email or password' => 'Incorrect email or password, or account is deactivated.',
    'Successfully authenticated' => 'Successfully authenticated.',
    'Successfully refreshed' => 'Successfully refreshed.',
    'Successfully registered' => 'Successfully registered.',
    'Role created successfully' => 'Role Created Successfully',
    'Role updated successfully' => 'Role Updated Successfully',
    'Role deleted successfully' => 'Role Deleted Successfully',
    'Role cannotBeDeleted' => "Role Can't Be Deleted",
    'Something went wrong' => 'Something went wrong.',
    'charge successfully' => 'Charge Successfully',
    'created successfully and wait review' => 'created successfully and wait for review',
    'Student created successfully' => 'Student created successfully.',
    'Student updated successfully' => 'Student updated successfully.',
    'Student deleted successfully' => 'Student deleted successfully.',
    'User created successfully' => 'User created successfully.',
    'User updated successfully' => 'User updated successfully.',
    'User deleted successfully' => 'User deleted successfully.',
    'Admin created successfully' => 'Admin created successfully.',
    'Admin updated successfully' => 'Admin updated successfully.',
    'Admin deleted successfully' => 'Admin deleted successfully.',
    'Field created successfully' => 'Field created successfully',
    'Field updated successfully' => 'Field updated successfully',
    'Field deleted successfully' => 'Field deleted successfully',
    'offer_added_successfully' => 'offer added successfully',
     'send_successfully' => 'send successfully',
     'user_not_found' => 'user not found',
    'Field cannotBeDeleted' => "Field Can't Be Deleted",
    'created successfully' => 'Created successfully.',
    'updated successfully' => 'Updated successfully.',
    'deleted successfully' => 'Deleted successfully.',
    'You have Increase Point' => 'You have Increase Point   ',
    'You have Decrease Point' => 'You have Decrease Point   ',
    'Point' => 'Point',
    'For' => ' For ',
    'Category created successfully' => 'Category created successfully',
    'Category updated successfully' => 'Category updated successfully',
    'Category deleted successfully' => 'Category deleted successfully',
    'Category cannotBeDeleted' => "Category Can't Be Deleted",
    'Standard created successfully' => 'Standard created successfully.',
    'Standard updated successfully' => 'Standard updated successfully.',
    'Standard deleted successfully' => 'Standard deleted successfully.',
    'Lecture created successfully' => 'Lecture created successfully.',
    'Lecture updated successfully' => 'Lecture updated successfully.',
    'Lecture deleted successfully' => 'Lecture deleted successfully.',
    'Course created successfully' => 'Course created successfully',
    'Course updated successfully' => 'Course updated successfully',
    'Course deleted successfully' => 'Course deleted successfully',
    'Course cannotBeDeleted' => "Course Can't Be Deleted",
    'Course created successfully , Please Assign Standards' => 'Course created successfully , Please Assign Standards',
    'Field must be checked' => ':field must be checked.',
    'Book created successfully' => 'Book Created Successfully',
    'Book updated successfully' => 'Book Updated Successfully',
    'Book deleted successfully' => 'Book Deleted Successfully',
    'Attachment created successfully' => 'Attachment Created Successfully',
    'Attachment updated successfully' => 'Attachment Updated Successfully',
    'Attachment deleted successfully' => 'Attachment Deleted Successfully',
    'Exam created successfully' => 'Exam Created Successfully',
    'Exam updated successfully' => 'Exam Updated Successfully',
    'Exam deleted successfully' => 'Exam Deleted Successfully',
    'Exam cannotBeDeleted' => "Exam Can't Be Deleted",
    'Coupon created successfully' => 'Coupon Created Successfully',
    'Coupon updated successfully' => 'Coupon Updated Successfully',
    'Coupon deleted successfully' => 'Coupon Deleted Successfully',
    'Question created successfully' => 'Question created Successfully.',
    'Question updated successfully' => 'Question updated Successfully.',
    'Question deleted successfully' => 'Question deleted Successfully.',
    'request_status_changed' => 'Print Request Status Changed Successfully.',
    'PaymentMethod confirmed successfully' => 'PaymentMethod confirmed successfully.',
    'wrong credentials' => 'Wrong Credentials Or your account need to be activated',
    'Password Incorrect' => 'Password Incorrect',
    'Exam Started Successfully' => 'Exam Started Successfully.',
    'You are not authorized to start this exam' => 'You are not authorized to start this exam.',
    'You are not authorized to end this exam' => 'You are not authorized to end this exam.',
    'Exam ended successfully' => 'Exam ended successfully.',
    'You are not authorized to perform this exam' => 'You are not authorized to perform this exam.',
    'You are not authorized to show this exam' => 'You are not authorized to show this exam.',
    'already subscribed' => 'Already Subscribed',
    'Added To Cart' => 'Added To Cart',
    'Phone is not valid' => 'Phone is not valid.',
    'Your profile updated successfully' => 'Your profile updated successfully.',
    'result_of_exam' => ':type result : :name',
    'question sent' => 'Question Sent',
    'lecture watched' => 'Lecture Watched',
    'You are not authorized to watch this lecture' => 'You are Not Authorized To Watch This Lecture',
    'already watched' => 'Already Watched',
    'Item added to cart successfully' => 'The :type added to cart successfully.',
    'Cannot add this item to cart' => 'Cannot add this :type to cart.',
    'Item is already in cart' => ':item is already in cart.',
    'You already have this course' => 'You already have this :item.',
    'Item removed from cart successfully' => 'Item removed from cart successfully.',
    'Cannot remove this item from cart' => 'Cannot remove this item from cart.',
    'Coupon is correct' => 'Coupon is correct.',
    'You have used this coupon before' => 'You\'ve used this coupon before.',
    'Coupon is invalid' => 'Coupon is invalid.',
    'You are not authorized to this course' => 'You Are Not Authorized To This Course',
    'Content published successfully' => 'Content published successfully.',
    'Solution Added successfully' => 'Solution Added successfully',
    'Solution updated successfully' => 'Solution updated successfully',
    'Solution deleted successfully' => 'Solution deleted successfully',
    'Subscription Added Successfully' => 'Subscription Added Successfully',
    'This item is on a previous payment that is being reviewed' => 'This :type is on a previous payment that is being reviewed.',
    'PaymentMethod declined successfully' => 'PaymentMethod declined successfully.',
    'Successfully loggedOut' => 'Successfully logged out',
    'rate_sent' => 'Rate Sent',
    'Course_Already_rated' => 'Course Already Rated',
    'Cart is empty' => 'Cart is empty.',
    'Answer saved successfully' => 'Answer saved successfully.',
    'You are not subscribed to the course of this exam' => 'You are not subscribed to the course of this exam.',
    'You have the same exam before not ended yet' => 'You have the same exam before not ended yet.',
    'The number of attempts available to enter the test has expired' => 'The number of attempts available to enter the test has expired.',
    'Your_message_has_been_sent' => 'Your Message Has Been Sent',
    'student_not_active' => "Student Account Is Not Active",
    'coupon has used max times' => 'Coupon Has Been Used Max Times Of Allowed Uses',
    'Coupon is not active with this items' => 'Coupon Is Not Available For This Items',
    'discount applied To' => 'Discount Applied To : ',
    'You are not authorized to embed this lecture' => 'You are not authorized to embed this lecture.',
    'You are not authorized to embed this introductory video' => 'You are not authorized to embed this introductory video.',
    'This course is not activated for you Contact with platform management' => 'This course is not activated for you. Feel free to contact with platform management.',
    'Subscription Deleted Successfully' => 'Subscription Deleted Successfully',
    'Registration Is Closed For This Course' => 'Registration Is Closed For This Course',
    'Bank created successfully' => 'Bank created successfully',
    'Bank updated successfully' => 'Bank updated successfully',
    'Bank deleted successfully' => 'Bank Deleted successfully',
    'Successful registration' => 'Successful registration.',
    'Failed registration' => 'Failed registration.',
    'Successful join request' => 'Your request to join the platform has been sent. The administration will respond to you via email as soon as possible.',
    'Failed join request' => 'Failed to send your join request.',
    'This user is not activated yet' => 'This user is not activated yet. Feel free to contact us.',
    'You are not authorized to access this Package' => 'You are not authorized to access this Package.',
    'Item was added to favourites successfully' => 'Item was added to favourites successfully.',
    'Item was deleted from favourites successfully' => 'Item was deleted from favourites successfully.',
    'There is an active subscription for this item' => 'There is an active subscription for this item',
    'Item not found' => 'Item not found.',
    'Your_message_has_been_deleted_for_violating_the_platform_rules_If_you_repeat_it_you_will_be_banned' => 'Your message has been deleted for violating the platform rules. If you repeat it, you will be banned.',
    'Your account is not activated yet' => 'Your account is not activated yet.',
    'Package initiated successfully' => 'Package initiated successfully.',
    'you are not authorized to do this action' => 'You are not authorized to do this action.',
    'The item has been added to the cart' => 'The item has been added to the cart.',
    'This item is already in cart' => 'This item is already in cart.',
    'You already have this item' => 'You already have this item.',
    'This item is being reviewed in past payment' => 'This item is being reviewed in past payment.',
    'This item is not accessible for you right now' => 'This item is not accessible for you right now.',
    'The item has been removed from the cart' => 'The item has been removed from the cart.',
    'PaymentMethod pending' => 'PaymentMethod is pending.',
    'PaymentMethod being_reviewed' => 'PaymentMethod is being reviewed.',
    'PaymentMethod confirmed' => 'PaymentMethod is confirmed successfully.',
    'PaymentMethod failed' => 'PaymentMethod failed.',
    'PaymentMethod refused' => 'PaymentMethod was refused.',
    'subscription will be ended today' => 'Subscription will be ended today.',
    'subscription will be in a day' => 'Subscription will be in a day.',
    'subscription will be ended in days' => 'Subscription will be ended in :days days',
    'You are not authorized to access this resource' => 'You are not authorized to access this resource.',
    'New subscription to' => 'New subscription to: :learnable',
    'Withdrawal request sent successfully' => 'Withdrawal request sent to administration to be reviewed.',
    'You' => 'You',
    'created_successfully' => 'Created successfully.',
    'deleted_successfully' => 'Deleted successfully.',
    'updated_successfully' => 'Updated successfully.',
    'Offer is approved successfully' => 'Offer is approved successfully.',
    'Delay close date must be after the original close date' => 'Delay close date must be after the original close date.',
    'OTP code has been sent' => 'OTP code has been sent.',
    'Your email has been verified successfully' => 'Your email has been verified successfully.',
    'Wrong OTP code' => 'Wrong OTP code.',
    'Offer has been submitted successfully' => 'The offer has been sent and will be published after review.',
    'You have already submitted an offer before' => 'You have already submitted an offer before.',
    'You are already subscribed to a Package' => 'You are already subscribed to a Package.',
    'Your payment and subscription have been completed successfully' => 'Your payment and subscription have been completed successfully.',
    'Your subscription will be activated after reviewing the payment details' => 'Your subscription will be activated after reviewing the payment details.',
    'Payment failed' => 'Payment failed',
    'This companys profile is not allowed to be shown' => 'This company\'s profile is not allowed to be shown.',
    'Your complaint has been sent successfully' => 'Your complaint has been sent successfully.',
    'Your message has been sent successfully' => 'Your message has been sent successfully.',
    'Password updated successfully' => 'Password updated successfully.',
    'This email is not existed' => 'This email is not existed.',
    'This purchase order cannot be delayed' => 'This purchase order cannot be delayed.',
    'The purchase order has been created and will be published after review' => 'The purchase order has been created and will be published after review.',
    'Your inquiry has been submitted and will be published after review' => 'Your inquiry has been submitted and will be published after review.',
    'sent successfully' => 'Sent successfully.',
    'Order has been placed successfully' => 'Order has been placed successfully.',
    'user_not_have_permission' => 'User Have Not Permission',
    'Wrong OTP code or expired' => 'Wrong OTP code or expired',
    'Your account has been verified successfully' => 'Your account has been verified successfully',
    'sended_successfully' => 'sended successfully',
    'the_wallet_amount_is_not_enough_pls_charge' => 'The Wallet Amount Is Not Enough Please Charge',
    'not_exist' => 'not exist' ,
     'old_password_is_not_correct' => 'Old Password Is Not Correct',
     'Your account is deactivated .' => 'Your account is deactivated .',
     'This order has been stopped for some reason.' => 'This order has been stopped for some reason.',
     'You are not allowed to access this resource' => 'You are not allowed to access this resource .',
     'You have a new price offer from ' => 'You have a new price offer from ',
     'You have a new update for the order : ' => 'You have a new update for the order : ',
     'You have a new comment from ' => 'You have a new comment from ',
    'You have a new customer signed up ' => 'You have a new customer signed up',
    'You have a new provider signed up ' => 'You have a new provider signed up',
    'You have a new complain' => 'You have a new complain',
    'You have a new blog' => 'You have a new blog',
    'You have a new question' => 'You have a new question',
    'You have a new order by customer : ' => 'You have a new order by customer : ',
    'not_exist' => 'not exist',
    'old_password_is_not_correct' => 'Old Password Is Not Correct',
    'Your account is deactivated .' => 'Your account is deactivated .',
    'This order has been stopped for some reason.' => 'This order has been stopped for some reason.',
    'You are not allowed to access this resource' => 'You are not allowed to access this resource .',
    'You have a new price offer from ' => 'You have a new price offer from ',
    'You have a new update for the order : ' => 'You have a new update for the order : ',
    'You have a new comment from ' => 'You have a new comment from ',
    'You have a new order from ' => 'You have a new order from ',
    'There is new order in ' => 'There is new order in ',
    'Order was canceled Successfully' => 'Order was canceled Successfully',
    'New Notification' => 'New Notification' ,
    'Unauthenticated' => 'Unauthenticated' ,
    'new message ' => 'new message ',
    'Your offer is accepted' => 'Your offer is accepted',
    'The data has been sent successfully and is awaiting review by the administration.' => 'The data has been sent successfully and is awaiting review by the administration.' ,
    'SERVICE_IMAGES' => 'Service images updates' ,
    'MAIN_DETAILS' => 'Main details updates' ,
    'The entered number is incorrect. Check the number.' => 'The entered number is incorrect. Check the number.' ,
    'The phone number must be a valid Egyptian number (e.g., 01XXXXXXXXX).'=>'The phone number must be a valid Egyptian number (e.g., 01XXXXXXXXX).' ,

];
