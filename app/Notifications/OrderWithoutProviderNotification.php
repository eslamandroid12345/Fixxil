...
class OrderWithoutProviderNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toDatabase(): array
    {
        return [
            'en' => array_merge($this->paramters, [
                'title' => 'You have a new order without provider by customer  : ' . ($this->paramters['customer'] ?? ''),
                'order_id' => (string) $this->paramters['order_id'] ?? null
            ]),
            'ar' => array_merge($this->paramters, [
                'title' => 'لديك طلب جديد من : ' . ($this->paramters['customer'] ?? ''),
                'order_id' => (string) $this->paramters['order_id'] ?? null
            ]),
        ];
    }
