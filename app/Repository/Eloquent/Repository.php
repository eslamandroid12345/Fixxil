...
abstract class Repository implements RepositoryInterface
{
    ...
    public function getActive(string $column = 'is_active', array $columns = ['*'], array $relations = []): Collection
    {
        return $this->model->with($relations)->where($column, true)->get($columns);
    }
    ...
    public function getActiveById(
        ...
        return $this->model->select($columns)->where($column, true)->with($relations)->findOrFail($modelId)->append($appends);
    }
    ...
    {
        return $this->model::query()->where($byColumn, $value)->count();
    }
}
